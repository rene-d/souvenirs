...
begin
  ...
  begin
    ...
    y_Axe(0,0,1);
    Couleur(blanc);
    for i:=0 to n do with P[i] do Croix(x,y);
    Couleur(brillant);
  end;
...
begin
  ...
  TraceBezier(true);
  Couleur(rouge);
  eps:=1/50;             { deux fois moins de points que pour la B-spline }
  x:=P[0].x-0.1;
  Deplace(x,y);
  repeat
    x:=x+eps;
...
begin
  ...
  TraceBezier(true);
  eps:=1/50;        { deux fois moins de points que pour la 1ère B-spline }
  w[0]:=0.5;
  Couleur(jaune);
  Deplace(-2,-2.5);
  Ecris('variation de w[0]');
  co:=bleu+1;
  repeat
    w[1]:=sqrt(w[0]*w[2]/C);
    Couleur(co); co:=co+1;
