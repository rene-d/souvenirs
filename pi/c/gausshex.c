/*
    ...
    GAUSSHEX.C
    version 1.2 du 19 octobre 1991
...
void    affiche(word *reel, FILE *stream); 	/* affiche un nombre réel */
...
int main(int argc, char *argv[])
{
    ...
    register int     i,j;	/* variables de calcul */
    register dword   r, tmp;
    register udword  ur, utmp;
    int     test1,test2;
    ...
    fwrite(&pihex[1],sizeof(word),nbdec,stream);
    fclose(stream);
    ...
    if ((stream = fopen("pi.dec","wt")) == NULL) {
        printf("Impossible d'ouvrir le fichier.");
        exit(1);
    }
...
void affiche(word *reel, FILE *stream)
{
